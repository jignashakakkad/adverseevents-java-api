(function () {
  'use strict';
    ...
    .factory('portfolioIndicationService' , ['$window', '$state', 'localStorageService', 'portfolioService', portfolioIndicationService]);

    function portfolioIndicationService($window, $state, localStorageService, portfolioService) {
        ...
        function finalizeScoreSignal(tset){
            ...
            return { 'metric_reduced_pscount': totalPS, 'metric_reduced_totaldrugs': totalDrugs, 'metric_reduced_totalsignals': totalSignals, 'metric_reduced_avg_ror': (d3.mean(totalRor) > 0) ? d3.mean(totalRor) : 0.00};
        };
        ...
        function getGridData(IndicationData, summaryData) {
            return _.map(portfolioService.getStripped(IndicationData), function (d) {
