(function () {
  'use strict';
    ...
    .controller('PortfolioIndicationCtrl', ['$scope', '$log', '$state', '$controller', 'RxsignalSummaryData', 'portfolioService', 'IndicationData', 'portfolioIndicationService', 'GridModel', PortfolioIndicationCtrl]);

    function PortfolioIndicationCtrl($scope, $log, $state, $controller, RxsignalSummaryData, portfolioService, IndicationData, portfolioIndicationService, GridModel) {
        ...
        var gridModel = new GridModel(currentState, 'portfolioIndication', false);

        vm.summaryData = portfolioService.getStripped(RxsignalSummaryData);
        vm.indicationData = portfolioIndicationService.getGridData(IndicationData, vm.summaryData);
        vm.title = 'Indications';

        gridModel.setData(vm.indicationData);
