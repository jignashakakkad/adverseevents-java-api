(function () {
  'use strict';
    ...
    .controller('PortfolioRxscoreCtrl', ['$scope', '$state', '$controller', 'portfolioService', 'RxscoreData', 'tooltipService', 'portfolioRxScoreService', 'GridModel', PortfolioRxscoreCtrl]);

    function PortfolioRxscoreCtrl($scope, $state, $controller, portfolioService, RxscoreData, tooltipService, portfolioRxScoreService, GridModel) {
        ...
        var gridModel = new GridModel(currentState, 'portfolioRxScore', false);

        vm.rxscoreData = portfolioService.getStripped(RxscoreData);
        vm.title = 'Product RxScore Breakdown';

        gridModel.setData(vm.rxscoreData);
