angular.module('rxsignal').controller('RxsignalBubbleChartCtrl', function ($scope, utilService, $http, rxsignalDataService, $stateParams,SERVER_SETTINGS) {
    ...
    //TODO(@chore) MOVE THIS INTO A FACTORY. I DONT THINK WE ARE USING ENOUGH DIFFERENT LIBS FOR GETTING FROM HTTP YET.
    d3.json(SERVER_SETTINGS.BASE_URL+'lev/rxsignal/' + $scope.aedrug_id + '/events/chart/bubble/simple', function (error, response) {

        $scope.data = response;
        $scope.colors = ['#b2182b', '#d6604d', '#f4a582', '#fddbc7', '#f7f7f7', '#d1e5f0', '#92c5de', '#4393c3', '#4392c3', '#2166ac'];
        var columns = [];
        var mySize = {};
        var mySizeArray = [];
        var myXs = {};
        ...
        var bubbleSizes = function (sizes) {
            ...
            var sizesArray = _.pluck(_sizes, 'size');

            var reacsArray = _.pluck(_sizes, 'reac');
            var newSizes = [];
            _sizes.forEach(function (size) {

                var newSize = pointSize(Math.abs(size.size), sizesArray);
        ...
        var myPointSizes = [];
        var i = 0;
        var myColors = $scope.colorbrewer['YlOrRd']['9'].reverse();
        var myPts = bubbleSizes(response);
        response.forEach(function (d) {
            ...
            columns.push(myPs);
            myXs[myRor[0]] = myPs[0];
            mySize[d.reac] = d.incidence;
            myNameIndex[d.reac] = d.y;
            myPointSizes.push = {'reac': d.reac, 'size': d.incidence};
        ...
        var chart = c3.generate({
            ...
            axis: {
                x: {
                    ...
                    padding: {top: 200, left: 10, right: 100, bottom: 0}
                },
                y: {
                    label: 'ROR',
                    tick: {
