/**
 * Created by cerdman on 9/10/14.
 */


angular.module('drugclass').controller('rxscoreDrugclassComponentsChartCtrl',function($scope, Indication, termService, rxScoreService,$window){
...
angular.module('drugs').controller('rxscoreIndicationsComparisonChartCtrl',function($scope, Indication, termService, rxScoreService){
    var dta;
    $scope.Indication = Indication[0];
    var thisDrugscore = Math.abs($scope.Indication.rxscore.score);

    $scope.categories = ['A','B','C','D'];
    $scope.highcharts_fetchData = function () {
        var termCode = termService.getVal();
        var thisDrug = $scope.Indication.termName;

        var thisDrugscore = $scope.Indication.rxscore.value;
...
angular.module('drugs').controller('rxscoreIndicationComparisonNavCtrl', function ($scope, Indication, rxScoreService, utilService, termService, $timeout) {
    $scope.Indication = Indication[0];
    $scope.height = '600px';
