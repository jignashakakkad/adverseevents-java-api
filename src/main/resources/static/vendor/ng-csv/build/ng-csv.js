...
angular.module('ngCsv.directives').
  directive('ngCsv', ['$parse', '$q', 'CSV', '$document', '$timeout', '$window', function ($parse, $q, CSV, $document, $timeout, $window) {
    return {
      restrict: 'AC',
      scope: {
        data: '&ngCsv',
        globalData: '@ngCsvReference',
        filename: '@filename',
        header: '&csvHeader',
      ...
      controller: [
        ...
        '$transclude',
        '$window',
        function ($scope, $element, $attrs, $transclude, $window) {
          $scope.csv = '';
          ...
          $scope.buildCSV = function () {
            ...
            // @cerdman - Custom hook to handle ng-csv-reference to get a window[reference] val
            if ($scope.globalData !== ''){
              $scope.data = function(){
                return $window[$scope.globalData];
              }
            }
