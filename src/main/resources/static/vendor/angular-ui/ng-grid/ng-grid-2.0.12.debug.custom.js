...
var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
    ...
    self.showSortButtonUp = function() {
        ...
        if (self.sortDirection == undefined) {
            return 1;
        }
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    ...
    $scope.adjustScrollLeft = function (scrollLeft) {
        ...
        for (var i = 0; i < x; i++) {
            ...
            if (col.visible == false) {
                $(".checkBoxMarkVisibleColumns"+col.colDef.index).hide();
            } else {
                $(".checkBoxMarkVisibleColumns"+col.colDef.index).show()
            }
            if (col.visible !== false) {
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('menuTemplate.html',
    "<div class='ng-grid-buttons pull-right'><div ng-show=\"showColumnMenu || showFilter\"  class=\"ngHeaderButton\">\r" +
    "\n" +
    // @dbogart TODO - new grid buttons
    "<div><i popover-placement=\"right\" popover-append-to-body=\"true\" popover=\"Show/Hide Columns\" popover-trigger=\"mouseenter\" ng-click=\"toggleShowMenu()\" class=\"grid-settings-cog pull-right fa fa-2 fa-cog\" ></i></div>\r" +
    "\n" +
    "</div>\r" +
    "\n" +
    "</div>\r" +
    ...
    "    </div>\r" +
    "\n" +
    "    <div ng-show=\"showColumnMenu\" class=\"chooseColumnsContainer\">\r" +
    "\n" +
    "        <span class=\"ngMenuText\">{{i18n.ngMenuText}}</span>\r" +
    "\n" +
    "        <ul class=\"multiSelectUl\">\r" +
    "\n" +
    "            <li class=\"multiSelectLi\" ng-repeat=\"col in columns | ngColumns\">\r" +
    "\n" +
    "                <label><span class=\"multiSelect\"><div class=\"checkBoxContainer\"><div class=\"multiSelectItem\"><div class=\"acol\"><input ng-disabled=\"col.pinned\" type=\"checkbox\" class=\"checkbox \" ng-model=\"col.visible\"/>{{col.displayName | outcomesFilter}}</div><span ng-show=\"col.visible\" class=\"tickMark  ng-scope\">✔</span></div></div></span></label>\r" +
    "\n" +
